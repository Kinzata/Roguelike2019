...
public class FieldOfViewComputer
{

    // Function handler to accept x,y of tile and determine if it blocks light
    Func<int, int, bool> _blocksLight;

    // Function that knows how to set a tile as visible
    Action<int, int> _setVisible;

    Func<int, int, int> GetDistance;

    public FieldOfViewComputer(Func<int, int, bool> blocksLight, Action<int, int> setVisible, Func<int, int, int> getDistance)
    {
    ...
    public void Compute(LevelPoint origin, int rangeLimit)
    {
        _setVisible((int)origin.X, (int)origin.Y);
